...
/**
 * A mouse over area that can be used for menus or buttons which support easing
 * effect
 * 
 * @author kevin
 * @author eriq
...
public class CheckBox extends AbstractComponent {
	...
	/** The current normalToggleImage being displayed */
	private Image currentToggleImage;

	/** The normalToggleImage being displayed in normal state */
	private Image normalToggleImage;

	/** The normalToggleImage being displayed in mouseOver state */
	private Image mouseToggleOverImage;

	/** The normalToggleImage being displayed in mouseDown state */
	private Image mouseToggleDownImage;
	 ...
	 * @param toggleImage
	 *            The normalToggleImage to display
	 * @param x
	 *            The position of the area
	 * @param y
	 *            the position of the area
	 */
	public CheckBox(GUIContext container, Image image, Image toggleImage,
			int x, int y) {
		this(container, image, toggleImage, x, y, image.getWidth(), image
				.getHeight());
	}
	...
	public CheckBox(GUIContext container, Image image, Image toggleImage,
		...
		this(container, image, toggleImage, new Rectangle(x, y, width, height));
	}
	...
	public CheckBox(GUIContext container, Image image, Image toggleImage,
			Shape shape) {
		super(container);
		setLocation((int) shape.getX(), (int) shape.getY());
		area = shape;
		normalImage = image;
		currentImage = image;
		mouseOverImage = image;
		mouseDownImage = image;
		normalToggleImage = toggleImage;
		currentToggleImage = toggleImage;
		mouseToggleOverImage = toggleImage;
		mouseToggleDownImage = toggleImage;
	...
	public void render(GUIContext container, Graphics g) {
		if (isSelected()) {
			renderCheckBox(currentToggleImage, g);
		} else {
			renderCheckBox(currentImage, g);
		}
	}
	 ...
	 */
	public void renderCheckBox(Image image, Graphics g) {
		if (image != null) {
	...
	private void updateImage() {
		if (!over) {
			...
			currentToggleImage = normalToggleImage;
			currentColor = normalColor;
			state = NORMAL;
			mouseUp = false;
		} else {
			if (mouseDown) {
				if ((state != MOUSE_DOWN) && (mouseUp)) {
					...
					currentToggleImage = mouseToggleDownImage;
				...
				if (state != MOUSE_OVER) {
					...
					currentToggleImage = mouseToggleOverImage;
	...
	public void setNormalToggleImage(Image normalToggleImage) {
		this.normalToggleImage = normalToggleImage;
	}

	public void setMouseToggleOverImage(Image mouseToggleOverImage) {
		this.mouseToggleOverImage = mouseToggleOverImage;
	}

	public void setMouseToggleDownImage(Image mouseToggleDownImage) {
		this.mouseToggleDownImage = mouseToggleDownImage;
	}
}
